	      ...
	      try {	
	          ...
	          for (int i = 0; i < nList.getLength(); i++) {
	             ...
	             if (nNode.getNodeType() == Node.ELEMENT_NODE) {
	                ...
	                case "intparam": parameters.add(intparam(p, eElement)); break;
	                case "boolparam": parameters.add(boolparam(p, eElement)); break;
	                case "catparam": parameters.add(catparam(p, eElement)); break;
	                }
...
public void test() {
	...
	//csv file of params:
	File file = new File("D:/Derek/Imperial/plugin/params.xml");
	Parameter[] ps = parseFile(file);
	for (Parameter p : ps) {
