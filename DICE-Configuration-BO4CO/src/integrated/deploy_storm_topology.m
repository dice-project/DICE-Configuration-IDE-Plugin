...
for i=1:length(options_)
    ...
    config_str=strcat(config_str, extrastr, '-c', extrastr, options_{1,i}, '=', setting(i));
end

% make the topology reliable
%config_str=[config_str extrastr '-c topology.acker.executors=1']
config_str=strcat(config_str, extrastr, '-c topology.acker.executors=1');

cli='storm';
%cmd=[cli extrastr 'jar' extrastr application_.jar_file extrastr application_.class extrastr application_.name config_str]
cmd=strcat(cli, extrastr, 'jar', extrastr, application_.jar_file, extrastr, application_.class, extrastr, application_.name, config_str);
[ssh2_conn, response] = ssh2_command(ssh2_conn,char(cmd));
