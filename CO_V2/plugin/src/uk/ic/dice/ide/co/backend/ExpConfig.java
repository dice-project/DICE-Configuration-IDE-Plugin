...
import uk.ic.dice.ide.co.backend.Parameters.ParamType;
		...
		for (Parameter p : params) {
			if (p.type == ParamType.INTRANGE) {
				try {
					...
					MessageDialog.openError(Display.getCurrent().getActiveShell(), "Cannot process parameter",
							"cannot process parameter '" + p.name + "'");
				}
			}
