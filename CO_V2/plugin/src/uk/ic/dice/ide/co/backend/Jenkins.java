...
public class Jenkins {
	...
	// Jenkins url
	static String jenkinsUrl = null;

	// Build name
	static String jobName = null;

	// Build token
	static String buildToken = null;
	...
	public static void setJen(String jenkinsUrl) {
		Jenkins.jenkinsUrl = jenkinsUrl;
	}

	public static void setJob(String jobName) {
		Jenkins.jobName = jobName;
	}

	public static void setToken(String buildToken) {
		Jenkins.buildToken = buildToken;
	}

	private static boolean isSet() {
		return username != null && password != null && jenkinsUrl != null && jobName != null && buildToken != null;
	}
	...
	public static void request(File file) {
		...
		if (!isSet()) {
			MessageDialog.openError(Display.getCurrent().getActiveShell(), "Jenkins configuration invalid",
					"Jenkins configuration invalid");
			return;
		}
		...
		HttpEntity postentity = MultipartEntityBuilder.create()
				...
				.addTextBody("json", payLoad, ContentType.TEXT_PLAIN).addBinaryBody("file0", file).build();
